...
def train_and_evaluate(
  ...
  if jax.process_index() == 0:
      wandb.init(
          project="meanflow-lora-finetune",
          tags=["lora", "mf-b4", "50k-imagenet"],
          notes="LoRA fine-tuning MeanFlow DiT-B/4 on 50k ImageNet subset",
  ...
  use_lora = config.model.cls.startswith("LoRA_")
  
  if use_lora:
          # Create temporary base model with matching checkpoint structure
          ...
          # Merge base params into LoRA state
          merged_params = load_pretrained_into_lora(state.params, base_state.params)
          merged_ema = load_pretrained_into_lora(state.ema_params, base_state.ema_params)
          state = state.replace(params=merged_params, ema_params=merged_ema)
          del base_state, base_model
          
          print_lora_summary(state.params)
          log_for_0("Loaded pretrained weights with LoRA adapters")
  else:
