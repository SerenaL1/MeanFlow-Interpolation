...
class MeanFlow(nn.Module):
  ...
  def setup(self):
    ...
    # Check if using LoRA model
    if model_str.startswith("LoRA_"):
        from models import models_dit_lora
        net_fn = getattr(models_dit_lora, model_str)
        lora_rank = self.model_config.get('lora_rank', 8)
        lora_alpha = self.model_config.get('lora_alpha', 16.0)
